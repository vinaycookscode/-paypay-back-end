...
export class UserHelper {
    ...
    static deleteUser(userId: string, isActive = false ,callback) {
        User.findByIdAndUpdate(userId, {isActive: isActive}, (findAndUpdateError, updatedData) => {
    ...
    static updateUserDetails(id, _userInformation: IUserInterface, callback) {
        User.findByIdAndUpdate(id, _userInformation, { new: true }, (findAndUpdateError, updatedData) => {
            if (!findAndUpdateError) {
                userInternalResponse.errors = null;
                userInternalResponse.status = true;
                userInternalResponse.data = [];
                userInternalResponse.msg = 'User is updated';
            } else {
                userInternalResponse.errors = findAndUpdateError;
                userInternalResponse.status = false;
                userInternalResponse.data = [];
