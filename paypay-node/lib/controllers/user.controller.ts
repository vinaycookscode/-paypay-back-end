...
export class UserController {
	...
	static updateUserDetails(req: Request, res: Response) {
		UserBL.updateUserDetails(req, (blResponse) => {
			if (blResponse?.status)  {
				HTTPResponse.successResponse(res, blResponse);
			} else {
				HTTPResponse.failureResponse(res, blResponse);
	...
	static toggleUserStatus(req: Request, res: Response) {
		UserBL.toggleUserStatus(req, (blResponse) => {
			if (blResponse?.status)  {
				HTTPResponse.successResponse(res, blResponse);
			} else {
				HTTPResponse.getFailureResponse(res, blResponse);
